...
2. **H (Fourier basis output)** — strongest inductive bias, largest parameter reduction; start
   here for RDLinear.
3. **G (RFFT predictor)** — a more expressive frequency-domain alternative to H; try when H
   under-fits.
4. **I (patch projection)** — good on high-frequency, high-channel data (Weather, ETTm*).
5. **J (multi-scale decomposition)** — layer on top of H once a single-scale winner is picked.
